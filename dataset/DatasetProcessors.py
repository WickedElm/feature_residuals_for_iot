...
class ReplaceMissingValuesWithZero(DatasetProcessor):
    def __init__(self, target_cols=None):
        super().__init__()
        self.target_cols = target_cols
    ...
    def process(self, df):
        for col in self.target_cols:
            df[col] = df[col].apply(lambda x: 0.0 if x == '-' or x == np.nan else x)
            df[col] = df[col].astype(float)
        return df
...
class PrepareAttackBenignLabelIot23(DatasetProcessor):
    ...
    def process(self, df):
        df[self.target_col] = df[self.target_col].apply(lambda x: 0 if 'Benign' in x else 1)
        return df
...
class FilterByDate(DatasetProcessor):
    def __init__(self, target_col='timestamp', method='greater', target_date='1970-01-01'):
        super().__init__()
        self.target_col = target_col
        self.method = method
    ...
    def process(self, df):
        df['timestamp_tmp'] = pd.to_datetime(df.timestamp, unit='s')
        if self.method == 'greater':
            df = df[df.timestamp_tmp > self.target_date]
        df = df.drop(columns=['timestamp_tmp'])
        return df

class FilterByPercentage(DatasetProcessor):
    def __init__(self, percentage=0, method='greater'):
        super().__init__()
        self.percentage = percentage
        self.method = method

    def process(self, df):
        filter_index = math.ceil(df.shape[0] * self.percentage)
        if self.method == 'greater':
            df = df.iloc[filter_index:]
        if self.method == 'less':
            df = df.iloc[:filter_index]
        return df
