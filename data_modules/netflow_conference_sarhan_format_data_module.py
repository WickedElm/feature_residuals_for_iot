...
class NetflowConferenceSarhanFormatDataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_path='./',
        batch_size=32,
        total_rows_threshold=200000,
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            self.ds_train = SarhanFormatNetflowDataset(ds.training_data, self.transform, class_weights=class_weights)
            self.ds_val = SarhanFormatNetflowDataset(ds.validation_data, self.transform)
            self.ds_test = SarhanFormatNetflowDataset(ds.test_data, self.transform)
